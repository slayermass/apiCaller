...
export const apiCaller: ApiCallerType = (apiFunction, apiArguments, options) => {
  ...
  const pathMapKey = `${options?.domain ? `${options.domain}_` : ''}${options?.key?.toString() ?? hashCode(apiFunction.name)}`;
...
export const DEV_PROMISE_MAP = process.env.NODE_ENV === 'production' ? undefined : promiseMap;
